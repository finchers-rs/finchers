...
pub trait EndpointResultExt<A, B>: Endpoint<Output = Result<A, B>> + Sized {
    ...
    ///
    /// The term "unwrap" does not means that a panic will be caused if the value is an "Err".
    /// Instead, the error value is treated as an object of "HttpError" and it will be converted
    /// to an HTTP response.
